...
ANSWER_RULES: tuple[AnswerRule, ...] = (
    ...
    AnswerRule(
        label="Attacker source IP",
        # An aggregated finding beats raw logon volume. On a real Sherlock the SSH log
        # held internet background brute-force noise (111.36.147.188, 119.73.124.129)
        # which outranked the address that actually attacked the service — one host
        # opening 37,630 MongoDB connections in 74 seconds. Higher confidence than the
        # logon rule so the synthesized finding leads.
        patterns=(_c(r"attacker'?s? ip"), _c(r"source ip"), _c(r"ip address of the attack"),
                  _c(r"which ip"), _c(r"what ip"), _c(r"originat\w+ ip"), _c(r"remote ip"),
                  _c(r"scann?(?:ed|ing|er)"), _c(r"brute ?forc")),
        field=("source_ip",), formatter=fmt_plain,
        event_types=("db_connection_flood", "connection_flood"),
        confidence=0.9, prefer="most_common", exclude=_NOT_A_REMOTE_IP,
    ),
    AnswerRule(
        label="First connection from the attacker",
        patterns=(_c(r"when .*(?:first )?(?:connect|access|reach)"),
                  _c(r"time .*(?:first )?connect"), _c(r"start of the attack"),
                  _c(r"when did the (?:attack|scan|flood)")),
        field="@ts", formatter=fmt_utc,
        event_types=("db_connection_flood", "connection_flood"),
        confidence=0.85, prefer="first",
    ),
    AnswerRule(
        label="Connections from the attacker",
        patterns=(_c(r"how many connection"), _c(r"number of connection"),
                  _c(r"connection count"), _c(r"how many time.*connect")),
        field=("connections",), formatter=fmt_int,
        event_types=("db_connection_flood", "connection_flood"),
        confidence=0.85, prefer="first",
    ),
    AnswerRule(
        ...
        patterns=(_c(r"what port"), _c(r"which port"), _c(r"port .*(?:listen|expos|open)"),
                  _c(r"listening on")),
        field=("port", "address"), formatter=fmt_plain,
        event_types=("db_listen", "service_start"),
    ...
    AnswerRule(
        label="System hostname",
        # The registry rule above is Windows-only, so on a Linux collection "what is
        # the hostname" returned nothing at all despite every syslog line carrying it.
        patterns=(_c(r"host ?name"), _c(r"name of the (?:host|machine|computer|system|server)"),
                  _c(r"computer name"), _c(r"server name")),
        field="@host", formatter=fmt_plain,
        confidence=0.6, prefer="most_common",
        # Cloud images name the box after its private IP until something renames it,
        # and that autogenerated name is rarely the answer being asked for.
        exclude=frozenset({"localhost", "localhost.localdomain", "-", "unknown"}),
    ),
    AnswerRule(
